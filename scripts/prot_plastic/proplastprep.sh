...
cp ~/project/scripts/prot_plastic/prep_umb.sh plumed/.

# Copy a correct set of input files and the modified forcefield
cp ~/paper_cadherin/plastic/${plast}/${plast}${len}/plastic/plastic.itp .
...
sed -i "2s/.*/#SBATCH --job-name=${plast}${len}prot/" 16gpu_submit
